package com.stodger.springboot.redis.service;
...
public class RedisCacheService {
    ...
    public RedisCacheService(StringRedisTemplate redisTemplate, RedisTemplate<String, Serializable> redisCacheTemplate) {
    ...
    public <K, V> void add(K key, V value) {
        try {
            if (value != null) {
                redisTemplate
                        .opsForValue()
                        .set(DEFAULT_KEY_PREFIX + key, JSONUtil.toJsonStr(value));
            }
        } catch (Exception e) {
    ...
    public <K, V> void add(K key, V value, long timeout, TimeUnit unit) {
        try {
            if (value != null) {
                redisTemplate
                        .opsForValue()
                        .set(DEFAULT_KEY_PREFIX + key, JSONUtil.toJsonStr(value), timeout, unit);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw new RuntimeException("数据缓存至redis失败");
    ...
    public <K, V> List<V> getList(K key, Class<V> clazz) {
        ...
        if (!StrUtil.isEmpty(value)) {
            result = JSONUtil.toList(new JSONArray(value), clazz);
        }
        return result;
    }
